...
import load.test.api.PostOrderBuildAPI;
...
public class LunchCreateOrderCouponLoadTC extends BaseLoadTC {
	String postOrderBuildAPIStr;
	...
	public void test(JavaSamplerContext arg0) throws UnsupportedEncodingException, IOException, ClientProtocolException,
		...
		postCreateOrderAPI.getRequestObject().setCouponSnList(new ArrayList<String>());
		postCreateOrderAPI.getRequestObject().getCouponSnList().add(postOrderBuildAPIStr);
	...
	public void setupTest(JavaSamplerContext arg0) {
		...
		PostOrderBuildAPI postOrderBuildAPI = new PostOrderBuildAPI();
		postOrderBuildAPI.setHost(String.format("http://%s/gw/mishi.order.build/1.0", arg0.getParameter("host")));
		postOrderBuildAPI.setApiName("mishi.order.build");
		PostOrderBuildRequestObject postOrderBuildRequestObject = new PostOrderBuildRequestObject();
		postOrderBuildRequestObject.setDateType(Integer.parseInt(arg0.getParameter("dateType")));
		postOrderBuildRequestObject.setIsSimple(false);
		postOrderBuildRequestObject.setShopId(arg0.getParameter("shopId"));
		postOrderBuildRequestObject.setProductBiz(Integer.parseInt(arg0.getParameter("productBiz")));
		Goods buildGoods = new Goods();
		buildGoods.setName(arg0.getParameter("goodsName"));
		buildGoods.setCount(Integer.parseInt(arg0.getParameter("goodsCount")));
		buildGoods.setGoodsId(arg0.getParameter("goodsId"));
		buildGoods.setPrice(Integer.parseInt(arg0.getParameter("goodsPrice")));
		buildGoods.setActivityGoodsId(0);
		List<Goods> buildGoodsList = new ArrayList<Goods>();
		buildGoodsList.add(buildGoods);
		postOrderBuildRequestObject.setGoodsList(buildGoodsList);
		postOrderBuildAPI.setRequestObject(postOrderBuildRequestObject);
		result += new Gson().toJson(postOrderBuildRequestObject);
		postOrderBuildAPI.serializeRequestObject(postOrderBuildAPI.getRequestObject());
		try {
			postOrderBuildAPI.sendRequest(postLoginResponseObject.getCookieStore());
		} catch (ClientProtocolException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
		...
		result += postOrderBuildAPI.getEntitystr();
		postOrderBuildAPIStr = postOrderBuildAPI.getEntitystr();
		postOrderBuildAPIStr = postOrderBuildAPIStr.substring(postOrderBuildAPIStr.indexOf("couponSn"));
		postOrderBuildAPIStr = postOrderBuildAPIStr.substring(postOrderBuildAPIStr.indexOf(":\""));
		postOrderBuildAPIStr = postOrderBuildAPIStr.substring(2, 8);
		result += postOrderBuildAPIStr;
	}
